...
#include <ctime>
  ...
  int main(){
     ...
     double integrate( double *& func , int Nx , int Ny, double dx , double dy );
     std::ofstream file ;
     std::clock_t clock();
     double t1 ;
     double t2 ;
...
//     Ny   =   1000 ;
    ...
    for ( int i = 0 ; i < Nx * Ny ; i++ ){
       file << x[ i ][ 0 ]  <<  "   "  <<  x[ i ][ 1 ]  <<  "   " << func[ i ] << std::endl;
    }

   
    std::cout << std::endl; 
    std::cout << "Integral: " << std::endl ;
    std::cout << "Hmm I hope it is okay..." << std::endl ;
    std::cout << integrate( func , Ny , Nx, dtheta , dphi ) << std::endl ;
    std::cout << std::endl ;
