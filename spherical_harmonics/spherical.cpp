...
int main(){
  ...
  void deallocate2d( double **& x , int N1 ) ;
  ...
  write_complex( grid , phi , spherical_harm , N , N2 , 4 );
 ...
 file.open( "spherical_harmonics.out" ) ; 
  ...
  for ( int i = 0 ; i < N ; i++ ){
    ...
    for ( int k = 0 ; k < N2 ; k++ ){
      file << x[ i ] << "    " << y[ k ] << "   " ; 
      for ( int j = 0 ; j < M ; j++ ){
        file << real( data[ row ][ j ] ) << "   " << imag( data[ row ][ j ] );
        file << "    ";
      }
      file << std::endl ;
      row++; 
...
//       for ( int i = 0 ; i < Ntheta ; i++ ){
//          for ( int j = 0 ; j < Nphi ; j++ ){
//             //spherical_harm[ row ][ col ]  =  norm * legendre[ i ][ col ] * exp( imagi * ( double ) m * phi[ j ] ) ;
//             spherical_harm[ row ][ col ]  =  exp( imagi * ( double ) m * phi[ j ] ) ;
//             row++;
//          }
//       }
//       col++;
